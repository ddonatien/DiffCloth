...
def run_step3(gp1_id, x_fin1, v_fin1):
    ...
    grasp2_traj = get_grasp2_traj(grasp_point_2, target, scene.stepNum)

    grasp_trajs = torch.cat((grasp1_traj, grasp2_traj), dim=1)

    x, v = stepSim(sim_mod, sim, (x_fin1.flatten(), v_fin1.flatten()), grasp_trajs)
    # x, v = stepSim(sim_mod, sim, (torch.tensor(state_info_init.x),
...
if __name__=="__main__":
   x1, v1, gp_id  = run_step2()
   x2, v2, gp_ids = run_step3(gp_id, x1[-1], v1[-1])
